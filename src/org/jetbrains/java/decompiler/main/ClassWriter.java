...
public class ClassWriter {
  ...
  private boolean isAutogeneratedConstructor(StructClass cl, StructMethod mt, TextBuffer code) {
    if (cl.getRecordComponents() == null) {
      return false;
    }
  ...
  private boolean isAutogeneratedGetter(StructClass cl, StructMethod mt, TextBuffer code) {
    if (cl.getRecordComponents() == null) {
      return false;
    }
    String name = mt.getName(), descriptor = mt.getDescriptor();
    StructRecordComponent correspondingRecord = cl.getRecordComponents().stream()
      .filter(c -> c.getName().equals(name))
      .findFirst()
      .orElse(null);
    if (correspondingRecord != null) {
      String desc = correspondingRecord.getDescriptor();
      if (desc != null && descriptor.equals("()" + desc)) {
        if (code.countLines() == 1) {
          String str = code.toString().trim();
        ...
        if (root != null && !methodWrapper.decompiledWithErrors) { // check for existence
          try {
            ...
            hideMethod = code.length() == 0 &&
              ...
              DecompilerContext.getOption(IFernflowerPreferences.EXCLUDE_AUTOGENERATED_METHODS) &&
                (isAutogeneratedConstructor(cl, mt, code) || isAutogeneratedGetter(cl, mt, code));
